...
void RetopoApp::initVulkan() {
    ...
    resourceManager = std::make_unique<ResourceManager>(device, physicalDevice);
    ...
    resourceManager->createDepthResources(swapChainExtent, depthFormat, depthImage, depthImageMemory, depthImageView);
    ...
    // --- MÓDOSÍTVA: A modellek és bufferek betöltése a ResourceManageren keresztül ---
...
void RetopoApp::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        swapChainImageViews[i] = resourceManager->createImageView(swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
    }
}
